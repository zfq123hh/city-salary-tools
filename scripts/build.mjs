...
function cityBonusCalc(city){return `<section class="card calculator" data-calculator="bonus"><div><h2>${city.name}年终奖个税计算器</h2><p class="muted">输入全年一次性奖金金额，按年终奖单独计税口径估算应纳个税和税后奖金。</p><div class="form-row"><label>全年一次性奖金</label><input name="bonus" type="number" value="${Math.round(city.avgSalary * 2)}" min="0" /></div><p class="notice">默认奖金参考 ${city.name} 平均工资的 2 个月估算，可手动改成实际年终奖金额。</p></div><div class="result"><p>预计税后奖金</p><p class="big js-net">--</p><dl><dt>预计个税</dt><dd class="js-tax">--</dd><dt>综合税负</dt><dd class="js-rate">--</dd></dl></div></section>`}
...
function cityIndex(){return layout({route:"/cities/",title:"城市工资计算器大全",description:"按城市查询税后工资、社保、公积金、年终奖个税估算工具。",keywords:["城市工资计算器","税后工资","社保基数","年终奖个税"],body:`<main class="container section">${breadcrumb([{label:"城市工资"}])}<h1>城市工资计算器大全</h1><p class="muted">已覆盖 ${cities.length} 个核心城市，每个城市包含税后工资、社保、公积金、年终奖个税四类长尾页面。</p><div class="grid">${cities.map(c=>`<article class="card"><h2>${c.name}</h2><p>${c.province} · 参考平均工资 ${c.avgSalary} 元</p><p><a class="btn" href="/cities/${c.slug}/salary-tax-calculator/">税后工资</a></p><p><a href="/cities/${c.slug}/social-security-calculator/">社保计算器</a> · <a href="/cities/${c.slug}/housing-fund-calculator/">公积金计算器</a> · <a href="/cities/${c.slug}/annual-bonus-tax-calculator/">年终奖个税</a></p></article>`).join("")}</div></main>`})}
